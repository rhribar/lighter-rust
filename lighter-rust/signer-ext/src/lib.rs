...
pub extern "C" fn CreateAuthToken(
    ...
    into_str_or_err(mgr.create_auth_token(deadline, account_index, api_key_index as u8, true))
}
...
#[no_mangle]
pub extern "C" fn SignJsonData(private_key: *const c_char, json_data: *const c_char, tx_type: c_int, chain_id: c_int) -> StrOrErr {
    let pk = unsafe { CStr::from_ptr(private_key) }.to_string_lossy().to_string();
