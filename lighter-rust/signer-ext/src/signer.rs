...
use goldilocks_crypto::{
    schnorr::{sign_with_nonce, verify_signature},
    Goldilocks, ScalarField,
};
...
impl KeyManager {
    ...
    pub fn create_auth_token(&self, deadline: i64, account_index: i64, api_key_index: u8, verify_sign: bool) -> Result<String> {
        ...
        if verify_sign {
            ...
            if !sig_ok {
                ...
                return Err(SignerError::Crypto(goldilocks_crypto::CryptoError::InvalidSignature));
            }
        }
    ...
    pub fn sign_transaction(&self, tx_json: &str, tx_type: u32, lighter_chain_id: u32, verify_sign: bool) -> Result<String> {
        ...
        let elements = match tx_type {
            ...
            8 => {
                ...
                let pubkey_bytes = hex::decode(pubkey_hex).map_err(|e| SignerError::API(format!("Invalid PubKey hex: {}", e)))?;
