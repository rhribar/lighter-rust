...
const N: [u64; 4] = [0x8c46eb2100000001, 0x224698fc0994a8dd, 0x0000000000000000, 0x4000000000000000];
const N0I: u64 = 0x8c46eb2100000001;
const R2: [u64; 4] = [0x07717a21e77894e8, 0x1a75e45b33d469f4, 0xc4dfc927c5ed3713, 0x2f431806ad2fe478];

// Elliptic curve constants
#[allow(dead_code)]
const A_ECG_FP5_POINT: Fp5Element = Fp5Element([Goldilocks(2), Goldilocks(0), Goldilocks(0), Goldilocks(0), Goldilocks(0)]);
pub const B_ECG_FP5_POINT: Fp5Element = Fp5Element([Goldilocks(0), Goldilocks(263), Goldilocks(0), Goldilocks(0), Goldilocks(0)]);
pub const B_MUL2_ECG_FP5_POINT: Fp5Element = Fp5Element([Goldilocks(0), Goldilocks(526), Goldilocks(0), Goldilocks(0), Goldilocks(0)]);
pub const B_MUL4_ECG_FP5_POINT: Fp5Element = Fp5Element([Goldilocks(0), Goldilocks(1052), Goldilocks(0), Goldilocks(0), Goldilocks(0)]);
pub const B_MUL16_ECG_FP5_POINT: Fp5Element = Fp5Element([Goldilocks(0), Goldilocks(4208), Goldilocks(0), Goldilocks(0), Goldilocks(0)]);
...
const GENERATOR_ECG_FP5_POINT: Point = Point {
    ...
    z: Fp5Element([Goldilocks(1), Goldilocks(0), Goldilocks(0), Goldilocks(0), Goldilocks(0)]),
    u: Fp5Element([Goldilocks(1), Goldilocks(0), Goldilocks(0), Goldilocks(0), Goldilocks(0)]),
    t: Fp5Element([Goldilocks(4), Goldilocks(0), Goldilocks(0), Goldilocks(0), Goldilocks(0)]),
};
...
impl Point {
    ...
    pub fn neutral() -> Self {
        Point::new(Fp5Element::zero(), Fp5Element::one(), Fp5Element::zero(), Fp5Element::one())
    }
    ...
    pub fn set_m_double(&self, n: u32) -> Point {
        ...
        for _i in 2..n {
            ...
            w_acc = t4
                .add(&t2.mul(&B_MUL4_ECG_FP5_POINT.sub(&Fp5Element::from_uint64_array([4, 0, 0, 0, 0]))))
                .neg();
        }
...
pub fn sign_with_nonce(private_key: &[u8], message: &[u8], nonce_bytes: &[u8]) -> Result<Vec<u8>> {
    ...
    let private_scalar = ScalarField::from_bytes_le(private_key).map_err(|_| CryptoError::InvalidPrivateKeyLength(private_key.len()))?;
    ...
    let nonce_scalar = ScalarField::from_bytes_le(&nonce_bytes_40).map_err(|_| CryptoError::InvalidPrivateKeyLength(nonce_bytes.len()))?;
...
pub fn verify_signature(signature: &[u8], message: &[u8], public_key: &[u8]) -> Result<bool> {
    ...
    // Convert to scalars
    let s = ScalarField::from_bytes_le(s_bytes).map_err(|_| CryptoError::InvalidSignatureLength(signature.len()))?;
    let e = ScalarField::from_bytes_le(e_bytes).map_err(|_| CryptoError::InvalidSignatureLength(signature.len()))?;
    ...
    let private_scalar = ScalarField::from_bytes_le(public_key).map_err(|_| CryptoError::InvalidPrivateKeyLength(public_key.len()))?;
    let generator = Point::generator();
